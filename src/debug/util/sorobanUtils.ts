...
/**
 * Helper functions for converting hex/base64 strings to Uint8Array for bytes types.
 * These functions enable the debugger to accept both hex (with optional 0x prefix)
 * and base64 encoded strings for any bytes/DataUrl parameter, matching CLI behavior.
 */

/** Regex pattern to match hex strings with optional 0x prefix */
const HEX_REGEX = /^(?:0[xX])?[0-9a-fA-F]+$/;

/**
 * Normalizes a hex string by removing the 0x or 0X prefix if present.
 * @param value - Hex string that may have a 0x prefix
...
/**
 * Checks if a string is a valid hex string (even length, valid hex chars, optional 0x prefix).
...
/**
 * Converts a hex or base64 string to Uint8Array.
 * Automatically detects hex strings (with or without 0x prefix) and converts them.
 * Falls back to base64 decoding if not hex.
 * @param value - Hex string (e.g., "dd11c3fa..." or "0xdd11c3fa...") or base64 string
 * @returns Uint8Array representation of the bytes
 */
const bytesStringToUint8Array = (value: string): Uint8Array => {
  const trimmed = value.trim();
...
const convertObjectToScVal = (obj: Record<string, unknown>): xdr.ScVal => {
  ...
  for (const key in obj) {
    const item = obj[key] as AnyObject | undefined;
    ...
    if (
      ...
      convertedValue[key] = item.value === "true";
      ...
      convertedValue[key] = bytesStringToUint8Array(item.value);
      typeHints[key] = ["symbol", "bytes"];
    } else {
      convertedValue[key] = item.value;
      typeHints[key] = ["symbol", item.type];
    }
  }
...
const convertTupleToScVal = (tupleArray: TupleValue[]) => {
  const tupleScVals = tupleArray.map((v) => {
    ...
    if (v.type === "bytes" && typeof v.value === "string") {
      return nativeToScVal(bytesStringToUint8Array(v.value));
    }
      ...
      if (isVecArray) {
        const arrayScVals = argValue.reduce((acc, v) => {
          if (v.type === "bool") {
            ...
            acc.push(bytesStringToUint8Array(v.value as string));
          } else {
            acc.push(v.value);
          }
          return acc;
